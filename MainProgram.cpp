...
public:
    ...
    BankAccount(const string& owner, double initialBalance) {
        ...
        if(owner.empty()){
            throw invalid_argument("Owner name can't be empty");    
        }
        if(initialBalance < 0){
            throw invalid_argument("Initial balance can't be below zero");   
        }
        owner_=owner;
    ...
    void deposit(double amount) {
        // TODO: Implement
        if(amount <= 0){
            throw invalid_argument("The deposit amount can't be below zero");     
        }
        balance_ += amount;
    }
    ...
    void withdraw(double amount) {
        // TODO: Implement
        if(amount <= 0){
            throw invalid_argument("The withdrawal amount can't be below zero");     
        }
        if(amount > balance_){
...
private:
    ...
    static void validate(const string& pwd) {
        ...
        if(pwd.length() < 8){
            throw invalid_argument("The password can't be shorter than 8 characters");    
        }
        if(!hasDigit(pwd)){
            throw invalid_argument("The password can't ");    
        }
    }
...
public:
    ...
    void change(const string& oldPassword, const string& newPassword) {
        ...
        if(oldPassword != password_){
             throw invalid_argument("The password can't be the same as the new one");    
        }
